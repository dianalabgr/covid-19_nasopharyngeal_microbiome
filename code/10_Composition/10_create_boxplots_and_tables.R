...
for(taxonomy_level in c("phylum","family","genus")){
  ...
  write.xlsx(as.data.frame(FINAL_TABLE)%>%select(-Feature.ID), paste0(tables_dir,"/Table_S1_",taxonomy_level,"_level_all_DA.xlsx"), row.names=FALSE, showNA=FALSE)
  ...
  FINAL_TABLE=merge(DA_taxa_direction, median_taxa_abundance, all.y=TRUE, by="Taxon") %>%
    arrange(Taxon) %>%
    arrange(`in severe compared to negative`)
  write.xlsx(as.data.frame(FINAL_TABLE)%>%select(-Feature.ID), paste0(tables_dir,"/Table_S1_",taxonomy_level,"_level.xlsx"), row.names=FALSE, showNA=FALSE)
  top_taxa=unique(FINAL_TABLE$Taxon)
  
#plot some selected taxa
  important_taxa=c("Actinobacteria","Firmicutes","Proteobacteria", "Fusobacteria","Bacteroidetes")
  if(taxonomy_level=="family"){important_taxa=c(
  ...
  if(taxonomy_level=="genus"){important_taxa=c("Corynebacterium","Elizabethkingia","Fusobacterium", 
                                               "Haemophilus", "Halomonas", "Prevotella", "Serratia","Streptococcus", "Veillonella")}
  
  queries <- paste(important_taxa, collapse = "|")
  OTUs_composition_table_important = OTUs_composition_table %>% 
  ...
  #create boxplot, important taxa only
  myboxplot=qplot(Taxon, Abundance, data = OTUs_composition_table_important, color = factor(type2, levels=c("1.negative","2.asymptomatic","3.mild","4.severe" )), geom = "boxplot", fill=factor(type2, levels=c("1.negative","2.asymptomatic","3.mild","4.severe")))+
